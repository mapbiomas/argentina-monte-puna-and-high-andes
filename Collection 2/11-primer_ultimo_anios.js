...
var Filter_5years = ee.Image(assetClass + 'CUYO-INTEGRADO-2-1sp-T3y-4y-5y');
...
var desmat = nivel0_2024.eq(10).and(nivel0_2023.eq(1))
...
Map.addLayer(Filter_5years.select('classification_2024'), visClass, 'Filter_5years (2024)', false);
Map.addLayer(class_final.select('classification_2024'), visClass, 'class_final (2024)', false);
Map.addLayer(class_final.select('classification_2024')
            .updateMask(class_final.select('classification_2024')
              .neq(Filter_5years.select('classification_2024'))
              ), visClass, 'cambio por correcciones (2024)', true);
Map.addLayer(Filter_5years.select('classification_1985'), visClass, 'Filter_5years (1985)', false);
Map.addLayer(class_final.select('classification_1985'), visClass, 'class_final (1985)', false);
Map.addLayer(class_final.select('classification_1985')
            .updateMask(class_final.select('classification_1985')
              .neq(Filter_5years.select('classification_1985'))
              ), visClass, 'cambio por correcciones (1985)', true);
...
Export.image.toAsset({
    "image": class_final,
    "description": 'CUYO-FINAL-2-1sp-T3y-4y-5y-1Ext' ,
    "assetId": assetClass + 'CUYO-FINAL-2-1sp-T3y-4y-5y-1Ext' ,
    "scale": 30,
    "pyramidingPolicy": {
        '.default': 'mode'
    },
